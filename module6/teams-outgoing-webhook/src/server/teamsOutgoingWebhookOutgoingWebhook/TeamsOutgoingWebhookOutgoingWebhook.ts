...
/**
 * Implementation for teams outgoing webhook Outgoing Webhook
 */
@OutgoingWebhookDeclaration("/api/webhook")
export class TeamsOutgoingWebhookOutgoingWebhook implements IOutgoingWebhook {
    ...
    private static processAuthenticatedRequest(incomingText: string): Partial<builder.Activity> {
        ...
        if (!selectedPlanet || !selectedPlanet.length) {
            ...
            const adaptiveCard = TeamsOutgoingWebhookOutgoingWebhook.getPlanetDetailCard(selectedPlanet[0]);
            message.type = "result";
    ...
    public requestHandler(req: express.Request, res: express.Response, next: express.NextFunction) {
        ...
        if (securityToken && securityToken.length > 0) {
            ...
            if (msgHash === auth) {
                ...
                const scrubbedText = TeamsOutgoingWebhookOutgoingWebhook.scrubMessage(incoming.text);
                message = TeamsOutgoingWebhookOutgoingWebhook.processAuthenticatedRequest(scrubbedText);
            } else {
