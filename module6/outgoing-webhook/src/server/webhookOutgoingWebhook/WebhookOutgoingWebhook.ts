...
/**
 * Implementation for webhook Outgoing Webhook
 */
@OutgoingWebhookDeclaration("/api/webhook")
export class WebhookOutgoingWebhook implements IOutgoingWebhook {
    ...
    private static processAuthenticatedRequest(incomingText: string): Partial<builder.Activity> {
        ...
        if (!selectedPlanet || !selectedPlanet.length) {
            ...
            const adaptiveCard = WebhookOutgoingWebhook.getPlanetDetailCard(selectedPlanet[0]);
            message.type = "result";
    ...
    public requestHandler(req: express.Request, res: express.Response, next: express.NextFunction) {
        ...
        if (securityToken && securityToken.length > 0) {
            ...
            if (msgHash === auth) {
                ...
                const scrubbedText = WebhookOutgoingWebhook.scrubMessage(incoming.text);
                message = WebhookOutgoingWebhook.processAuthenticatedRequest(scrubbedText);
